...
def generate_core_code_node(state: ResearchState) -> ResearchState:
    ...
    derivation_trace = state.get("derivation_trace", "")
    locked_method_spec = state.get("locked_method_spec", "")
    ...
    code_resp = llm.invoke([
        ...
        HumanMessage(content=(
            f"# Background\n"
            f"Broken assumption: {broken}\n\n"
            f"Locked method spec (the source of truth):\n{locked_method_spec[:1200]}\n\n"
            f"Narrative (reference only):\n{_extract_method_section(story)}\n\n"
            f"Derivation trace (claim→predictions→components):\n{derivation_trace[:800]}\n\n"
