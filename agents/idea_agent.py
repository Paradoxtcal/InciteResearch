...
def _fulltext_mode(state: ResearchState) -> str:
    v = (state.get("fulltext_mode") or os.environ.get("RESEARCH_AGENT_FULLTEXT_MODE") or "").strip().lower()
    if v in ("", "off", "none", "false", "0"):
        return "off"
    if v in ("topk", "top", "top-10", "top10", "k"):
        return "topk"
    if v in ("adaptive", "auto", "dynamic"):
        return "adaptive"
    return "off"


def _fulltext_int(state: ResearchState, key: str, env_key: str, default: int) -> int:
    v = state.get(key)
    if isinstance(v, int):
        return v
    try:
...
def _pick_topk_pdf_ids(papers: list[dict], k: int) -> list[str]:
    if k <= 0:
        return []
    out = []
    for p in papers or []:
...
def _pick_adaptive_pdf_ids(
    papers: list[dict],
    min_n: int,
    max_n: int,
    score_threshold: int,
) -> list[str]:
    cands = [p for p in (papers or []) if isinstance(p, dict) and p.get("pdf_url")]
    if not cands or max_n <= 0:
        return []

    has_score = any(p.get("relevance_score", None) is not None for p in cands)
    if not has_score:
        return _pick_topk_pdf_ids(cands, min(max_n, max(min_n, 0)))

    def _score(p: dict) -> int:
        try:
            return int(p.get("relevance_score", 0) or 0)
        except Exception:
            return 0

    cands_sorted = sorted(cands, key=_score, reverse=True)
    strong = [p for p in cands_sorted if _score(p) >= score_threshold]
    picked = strong[:max_n]
    if len(picked) < max(min_n, 0):
        for p in cands_sorted:
            if p in picked:
                continue
            picked.append(p)
            if len(picked) >= min(max_n, max(min_n, 0)):
                break
...
def paper_search_node(state: ResearchState) -> ResearchState:
    ...
    fulltext_mode = _fulltext_mode(state)
    if llm_ok and fulltext_mode != "off":
        k = _fulltext_int(state, "fulltext_k", "RESEARCH_AGENT_FULLTEXT_K", 10)
        min_n = _fulltext_int(state, "fulltext_min", "RESEARCH_AGENT_FULLTEXT_MIN", 4)
        max_n = _fulltext_int(state, "fulltext_max", "RESEARCH_AGENT_FULLTEXT_MAX", 18)
        score_th = _fulltext_int(state, "fulltext_score_threshold", "RESEARCH_AGENT_FULLTEXT_SCORE_THRESHOLD", 85)
        try:
            pdf_max_pages = int(os.environ.get("RESEARCH_AGENT_FULLTEXT_PDF_MAX_PAGES", "12"))
        except Exception:
            pdf_max_pages = 6
        try:
            pdf_max_chars = int(os.environ.get("RESEARCH_AGENT_FULLTEXT_PDF_MAX_CHARS", "12000"))
        except Exception:
            pdf_max_chars = 12000

        if fulltext_mode == "topk":
            picked = _pick_topk_pdf_ids(unique, k=k)
        else:
            picked = _pick_adaptive_pdf_ids(unique, min_n=min_n, max_n=max_n, score_threshold=score_th)
        ...
        if picked:
            ...
            for pid in picked:
                ...
                if not isinstance(paper, dict):
                    continue
                res = fetch_fulltext_excerpt(paper, max_pages=pdf_max_pages, max_chars=pdf_max_chars)
                if not res.get("ok"):
                    library[pid] = {**entry, "paper_id": pid, "fulltext_status": "unavailable", "pdf_url": res.get("pdf_url")}
                    continue
                library[pid] = {
            ...
            if updated:
                save_paper_library(library)
                print(f"  ✓ Auto-read and cached full-text excerpts for {updated} papers | mode={fulltext_mode}")

    if fulltext_mode == "off" and llm_ok and _auto_pdf_enabled() and not (state.get("paper_fulltext_request") or []):
        max_n = _auto_pdf_max()
