...
def story_arc_node(state: ResearchState) -> ResearchState:
    ...
    response = llm.invoke([
        ...
        HumanMessage(content=(
            ...
            "📌 **Predictions**: 2–3 testable predictions implied by the claim.\n"
            "🧱 **Constraints**: Minimal method requirements derived from the predictions.\n"
...
def derivation_trace_node(state: ResearchState) -> ResearchState:
    llm = _get_llm(temperature=0.2)
    response = llm.invoke([
        SystemMessage(content=DERIVATION_TRACE_PROMPT),
        HumanMessage(content=(
            f"Idea:\n{state.get('selected_idea', '')}\n\n"
            f"Broken assumption:\n{state.get('broken_assumption', '')}\n\n"
            f"Story arc:\n{state.get('story_arc', '')}\n\n"
            "Produce the derivation trace JSON."
        )),
    ])
    ...
    return {
        **state,
        "derivation_trace": trace_json,
    }


def method_lock_node(state: ResearchState) -> ResearchState:
    llm = _get_llm(temperature=0.2)
    response = llm.invoke([
        SystemMessage(content=METHOD_LOCK_PROMPT),
        HumanMessage(content=(
            f"Idea:\n{state.get('selected_idea', '')}\n\n"
            f"Broken assumption:\n{state.get('broken_assumption', '')}\n\n"
            f"Story arc:\n{state.get('story_arc', '')}\n\n"
            f"Derivation trace (JSON, may be partial):\n{state.get('derivation_trace', '')}\n\n"
            "Produce the locked method spec JSON."
        )),
    ])
    ...
    if match:
        try:
            data = json.loads(match.group())
            locked_json = json.dumps(data, ensure_ascii=False)
        except Exception:
            locked_json = content
    else:
        locked_json = content
    ...
    return {
        **state,
        "locked_method_spec": locked_json,
    }
...
def necessity_check_node(state: ResearchState) -> ResearchState:
    ...
    response = llm.invoke([
        ...
        HumanMessage(content=(
            f"Derivation trace (JSON, may be partial):\n{state.get('derivation_trace', '')}\n\n"
            f"Locked method spec (JSON, may be partial):\n{state.get('locked_method_spec', '')}\n\n"
            f"Story arc:\n{state.get('story_arc', '')}\n\n"
            ...
            "**(4) Anti-inversion test**\n"
            "Does anything look constructed to justify the hypothesis (post-hoc), e.g., components not needed by any prediction, or validation that cannot falsify the claim?\n"
            "If yes, specify exactly what to remove/replace and how to rewrite the story so the method becomes inevitable.\n\n"
            "**(5) Uniqueness test**\n"
            "Is the method uniquely implied by the insight? If not, pinpoint the remaining degrees of freedom and propose the smallest additional constraint to eliminate them.\n\n"
