...
def _autosave_partial(state: dict) -> None:
    ...
    _snapshot_write(session_id, "state", "json", state_text)
...
def check_env():
    ...
    ofox_key = os.environ.get("OFOX_API_KEY")
...
def run():
    ...
    _run_interactive(graph, config, last_ai=last_ai, last_saved=last_saved)


def _latest_ai_from_state(state: dict) -> str:
    msgs = state.get("messages", []) or []
    for m in reversed(msgs):
        if isinstance(m, dict) and m.get("role") == "assistant":
            c = (m.get("content", "") or "").strip()
            if c and not _is_internal_marker(c):
                return c
...
def _run_interactive(graph, config, last_ai: str, last_saved: dict):
    while True:
        try:
        ...
        if stopped_at == "direction_select":
            ...
            if raw_papers:
                ...
                pick = input("\n" + UI["pick_fulltext"] + "\n> ").strip()
                if pick.strip().lower() in ("auto", "adaptive"):
                    try:
                        min_n = int(os.environ.get("RESEARCH_AGENT_FULLTEXT_MIN", "4"))
                    except Exception:
                        min_n = 4
                    try:
                        max_n = int(os.environ.get("RESEARCH_AGENT_FULLTEXT_MAX", "18"))
                    except Exception:
                        max_n = 18
                    try:
                        th = int(os.environ.get("RESEARCH_AGENT_FULLTEXT_SCORE_THRESHOLD", "85"))
                    except Exception:
                        th = 85
                    paper_ids = _adaptive_pick_paper_ids(raw_papers, min_n=min_n, max_n=max_n, score_threshold=th, pool_n=30)
                else:
...
def run_resume(session_id: str):
    print("\n" + "="*58)
    print("  InciteResearch (resumed)")
    print("="*58)
    from agents.orchestrator import resume_session
    graph, config = resume_session(session_id)
    try:
        st = graph.get_state(config).values
    except Exception:
        print("\n❌ No saved checkpoint found for that session_id.")
        return
    _autosave_from_graph(graph, config)
    last_ai = _latest_ai_from_state(st) or ""
    if last_ai:
        print(f"\nAI: {last_ai}\n")
    last_saved = {"proposal": st.get("research_proposal", "") or "", "story": st.get("story_arc", "") or "", "dirs": 0}
    _run_interactive(graph, config, last_ai=last_ai, last_saved=last_saved)
...
def _adaptive_pick_paper_ids(
    ...
    if not papers or max_n <= 0:
        return []
    cands = [p for p in (papers[:pool_n] or []) if isinstance(p, dict) and p.get("pdf_url")]
    if not cands:
        return []
    ...
    if not has_score:
        out = []
        for p in cands[: min(max_n, max(min_n, 0))]:
            pid = p.get("paper_id")
            if pid:
                out.append(pid)
        return out
