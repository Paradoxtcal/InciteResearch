...
def normalize_plan_text(s: str) -> str:
    """Strip outer whitespace; same normalization used for threshold comparisons."""
    if not s:
        return ""
    return s.strip()


def count_plan_chars(plan: str) -> int:
    """Visible character count for PLAN length gates (must match extract_* output)."""
    return len(normalize_plan_text(plan))


def extract_fenced_block_after_label(text: str, label: str) -> str:
    """
    Extract ```{label} ... ``` without the classic non-greedy bug: inner ``` lines
    must not truncate the PLAN (common when the model embeds code blocks).
    Closing fence = a line containing only ``` (after strip).
    """
    ...
    if idx < 0:
        return ""
    rest = text[idx + len(opener) :].lstrip()
    if rest.startswith("\n"):
        rest = rest[1:]
...
def extract_prompt(text, word):
    robust = extract_fenced_block_after_label(text, word)
    if robust:
        return robust
    code_block_pattern = rf"```{re.escape(word)}(.*?)```"
