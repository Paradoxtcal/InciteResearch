    ...
    def command_descriptions(self, phase):
        ...
        if phase == "plan formulation":
            return (
                ...
                "For the final PLAN command, you MUST produce a long and detailed plan. The PLAN body must be at least 2500 Chinese characters (or about 1500+ English words), and include concrete implementation-level details.\n"
                "The PLAN must include clear section headings for at least: Problem Statement, Motivation, Method Overview, System Architecture, Algorithmic Steps, Data Pipeline, Implementation Plan, Evaluation Protocol, Ablations, Risks/Failure Modes, and Expected Outcomes.\n"
                ...
                "Do not end early with a short high-level sketch. Submit the PLAN only when it is comprehensive, actionable, and sufficiently long.\n"
    ...
    def phase_prompt(self, phase):
        ...
        if phase == "plan formulation":
            phase_str = (
                ...
                "CRITICAL: Do NOT output a generic 'Ablation Proposal' or use the previous template.\n"
                "Instead, when you output the final plan using the ```PLAN command, you MUST heavily concretize, detail, and fully operationalize the abstract concepts provided in the user's task notes.\n"
                "You must break down the abstract method into actionable algorithms, data structures, and code-level logic.\n"
                "Specify exactly HOW to build the system, what the inputs and outputs are, the exact execution flow, and step-by-step implementation details.\n"
                "Structure your plan clearly (e.g., using headings like 'System Architecture', 'Algorithmic Implementation', 'Execution Flow', 'Data Structures', 'Evaluation Metrics') to best explain the concrete realization of the specific abstract idea.\n"
                "LENGTH REQUIREMENT: your final PLAN must be long-form and highly detailed, at least 2500 Chinese characters (or about 1500+ English words). Short plans are invalid.\n"
            )
        elif phase == "results interpretation":
            phase_str = (
...
class PhDStudentAgent(BaseAgent):
    ...
    def format_review(self):
        ...
        for _l in self.lit_review:
            title = (_l.get("title") or "").strip()
            sid = (_l.get("arxiv_id") or "").strip()
            authors = (_l.get("authors") or "").strip()
            reason = (_l.get("relevance_reason") or "").strip()
            abstract = (_l.get("abstract_text") or _l.get("full_text") or "").strip()
            if not abstract:
                abstract = (_l.get("summary") or "").strip()
            excerpt = (_l.get("pdf_excerpt") or "").strip()
            lines: list[str] = []
            lines.append(f"• {title}" if title else f"• (untitled) ID {sid}")
            ...
            if abstract:
                ...
                lines.append("  Abstract: (none in review entry)")
            if excerpt:
                lines.append(
                    "  PDF excerpt (partial; same pipeline as main InciteResearch fetch_fulltext_excerpt; not whole paper):\n  "
                    + excerpt
                )
            if reason:
                lines.append(f"  Relevance (InciteResearch): {reason}")
            chunks.append("\n".join(lines))
